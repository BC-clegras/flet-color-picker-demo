import time
import flet
from flet import colors, icons, Container, Text, SnackBar, ProgressBar, ButtonStyle, IconButton, AppBar, Tabs, ListView, Tab, Page, UserControl, Column, TextField, Ref
...
class Tile(Container):
    def __init__(self, tile_text, color, page):
        ...
        self.color_text = tile_text
        self.color_val = f"colors.{tile_text}"
        self.bgcolor = color
        self.expand = True
        self.height = 40
        self.content = self.text
        self.page = page
        self.tooltip = "Click to set this color as background"

    def _build(self):
        def click_event(e):
            """
            Sets the clicked color as the background of the screen
            Adds the clicked color to the gradient's color list

            :param e: The event that triggered the function
            """
            self.page.show_snack_bar(SnackBar(Text(f"Set background to {self.color_text}!"), open=True))

            self.page.bgcolor = eval(self.color_val)
            self.page.demo_stack.gradient_demo.gradient.colors.append(eval(self.color_val))
...
class ColorPicker(UserControl):
    def __init__(self, page, expand=True, height=500, width=250):
        """
    ...
    def build(self):
        ...
        search_field = TextField(
            ...
            border_radius=50,
            suffix=IconButton(icon=icons.CHECK, bgcolor=colors.INVERSE_PRIMARY, icon_color=colors.ERROR, on_click=filter_tabs),
        ...
        return Column(controls=[search_field, Tabs(ref=self.displayed_tabs, expand=True, tabs=create_tabs(self.original_tab_names))])
...
def main(page: Page):
    ...
    # Creating an instance of the ColorPicker class and passing the page object to it.
    version_2 = ColorPicker(page)
